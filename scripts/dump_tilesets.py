...
tiletable = (0x26, 0x4000)
infotable = (0x26, 0x4800)
terminator = 0x4700 # The final address of the table, and also what's used to specify an empty entry
...
# M4 keeps source addresses in a table in bank 0 and banks + destination info in a separate table
# This is pretty much a single table that's been split over 2 banks, and there may be duplicate tileset entries
count = ((terminator - tiletable[1]) // 2)
...
for version in roms:
    ...
    with open(version[0], "rb") as rom:
        rom.seek(utils.rom2realaddr(tiletable))
        ...
        while i < count:
            ...
            # Unlike M3, it seems like M4 will sometimes have a valid bank even with a terminator pointer... which is rather confusing
            # TODO: We need to verify this, as it may just be that we don't have the correct means to decompress something...
            # 1F:4700 seems to be the only one
            if ptr == terminator:
                if realptr != terminator_key:
                    realptr_key_map[ver][realptr] = terminator
            elif realptr not in realptr_key_map[ver]:
...
''')
    ...
    if tiletable[0] == 0:
        output.write(f'SECTION "Tileset Source Address Table", ROM0[${tiletable[1]:04X}]\n')
    else:
        output.write(f'SECTION "Tileset Source Address Table", ROMX[${tiletable[1]:04X}], BANK[${tiletable[0]:02X}]\n')   
    ...
    for i, key in enumerate(tileset_metadata[default_version]):
        ...
        output.write(f"  TilesetTableEntry {name} ; {i:02X} ({(tiletable[1] + i * 2):04X})\n")
