...
for input_file in input_files:
    ...
    defined_variables = dict(l.replace(' ', '').replace('DEF', '').replace('$','').split('EQU') for l in lines if l.startswith('DEF'))
    for line in lines:
        if line.startswith('SECTION') and f'"Pointer List - {key}"' in line:
            o = line.lstrip('SECTION ').replace(' ', '').replace('\n','').replace('\r\n','').replace('"','').split(',')
            #Name ROMX[$OFFSET] BANK[$BANK]
            bank = o[2].replace('BANK','').replace('[','').replace(']','').replace('$','0x')
            ptr_table_offset = o[1].replace('ROMX','').replace('[','').replace(']','').replace('$','0x')

            if bank.startswith('cBank'):
                bank = int(defined_variables[bank], 16)
            else:
                bank = int(bank, 16)

            if ptr_table_offset.startswith('cAddress'):
                ptr_table_offset = int(defined_variables[ptr_table_offset], 16)
            else:
                ptr_table_offset = int(ptr_table_offset, 16)
